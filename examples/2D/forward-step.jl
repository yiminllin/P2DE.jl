    ...
    for k in 1:get_num_elements(param)
        for i in 1:Nfp
            ...
            if (abs(xf[i,k]) <= 1e-14 && i <= 2*(N+1))
                push!(mapI,idx)
                push!(inflowarr,primitive_to_conservative(equation,(1.4,3.0,0.0,1.0)))
            elseif (abs(xf[i,k]-3.0) <= 1e-14 && yf[i,k] >= 0.2 && i <= 2*(N+1))
                push!(mapO,idx)
                # push!(mapI,idx)
                # push!(inflowarr,primitive_to_conservative(equation,(1.4,3.0,0.0,1.0)))
            elseif (abs(xf[i,k]-3.0) <= 1e-14 && yf[i,k] < 0.2)
                push!(mapI,idx)
                push!(inflowarr,primitive_to_conservative(equation,(1.4,0.0,0.0,1.0)))
            elseif (abs(xf[i,k]-0.6) <= 1e-14 && yf[i,k] <= 0.2 && yf[i,k] >= 0.0 && i <= 2*(N+1)) 
                push!(mapWslip,idx)
                # push!(mapWnoslip,idx)
            elseif (abs(yf[i,k]-0.2) <= 1e-14 && xf[i,k] < 3.0-1e-14 && xf[i,k] >= 0.6 && i > 2*(N+1)) 
                push!(mapWslip,idx)
                # push!(mapWnoslip,idx)
            elseif (abs(yf[i,k]-0.0) <= 1e-14 && i > 2*(N+1))
                push!(mapWslip,idx)
                # push!(mapWnoslip,idx)
            elseif (abs(yf[i,k]-1.0) <= 1e-14 && i > 2*(N+1))
                push!(mapWslip,idx)
                # push!(mapWnoslip,idx)
            end
        end
...
CFL = 0.5
# K1D need to be multiple of 5
param = Param(N=3, K=(150,50), xL=(0.0,0.0), xR=(3.0,1.0),
...
construct_vtk_file!(caches.postprocessing_cache,param,data_hist,"/data/yl184/outputs/figures/forward-step","forward-step")
